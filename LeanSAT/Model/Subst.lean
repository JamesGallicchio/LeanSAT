...
def liftSubst (f : ν₁ → PropForm ν₂) : ν₁ → PropFun ν₂ :=
  (⟦PropForm.subst · f⟧)

instance : Coe (ν₁ → PropForm ν₂) (ν₁ → PropFun ν₂) := ⟨liftSubst⟩
...
theorem subst_liftSubst_eq_substL (f : ν₁ → PropForm ν₂) (φ : PropFun ν₁)
    : subst φ (liftSubst f) = substL φ f := by
  ext τ
  simp
  exact ⟨id, id⟩
